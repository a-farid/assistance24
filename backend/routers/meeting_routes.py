from settings.standarization import pagination_params, json_response # pyrefly: ignore [missing-import]
...
async def get_my_meetings(pagination: dict = Ds(pagination_params),token =Ds(jwt_s.roles_allowed(["worker", "client"]))):
    ...
    return json_response(**response, message="User meetings retrieved")
...
async def get_all_meetings(pagination: dict = Ds(pagination_params), _: dict =Ds(jwt_s.roles_allowed(["admin"]))):
    ...
    return json_response(**contracts, message="All meetings retrieved")
